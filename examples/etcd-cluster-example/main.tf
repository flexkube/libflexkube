...
resource "local_file" "config" {
  content = templatefile("./config-new.yaml.tmpl", {
    members    = var.peers
    peer_ca    = tls_locally_signed_cert.etcd_ca.cert_pem
    peer_certs = tls_locally_signed_cert.peer.*.cert_pem
    peer_keys  = tls_private_key.peer.*.private_key_pem
  })
